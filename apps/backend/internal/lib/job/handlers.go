...
func (j *JobService) handleWelcomeEmailTask(ctx context.Context, t *asynq.Task) error {
	...
	if err := json.Unmarshal(t.Payload(), &p); err != nil {
		return fmt.Errorf("failed to unmarshal welcome email paload: %w", err)
	}
	...
	j.logger.Info().
		Str("type", "welcome").
		Str("to", p.To).
		Msg("Successfullt sent welcome email")
	return nil
}
...
func (j *JobService) handleReminderEmailTask(ctx context.Context, t *asynq.Task) error {
	...
	if err := json.Unmarshal(t.Payload(), &p); err != nil {
		return fmt.Errorf("failed to uumarshal reminder email payload: %w", err)
	}
