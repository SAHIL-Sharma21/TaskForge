...
import (
	...
	"github.com/SAHIL-Sharma21/go-taskForge/internal/errs"
...
// extractColumnForUniqueVoilation gets field name from unique constraint
func extractColumnForUniqueVoilation(constraintName string) string {
	if constraintName == "" {
		return ""
	}
...
// HandleError processes a database error into and appropriate application error
func HandleError(err error) error {
	...
	if errors.As(err, &pgerr) {
		...
		// Generate an approriate error code and message
		...
		case UniqueViolation:
			columnName := extractColumnForUniqueVoilation(sqlErr.ConstraintName)
			if columnName != "" {
